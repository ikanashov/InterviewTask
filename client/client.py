...
from .config import ALLOWEDVIDEOEXTWEEKEND, ALLOWEDVIDEOEXTWORKDAY
from .config import DELTADAYSFORIGNORE, EMOJIDAYS
...
class Client():
    ...
    def process_video(self) -> str:
        if self.request.ts.isoweekday() > 5:
            return self.is_video_allow(
                self.request.content, ALLOWEDVIDEOEXTWEEKEND)
        return self.is_video_allow(
            self.request.content, ALLOWEDVIDEOEXTWORKDAY)
    ...
    def unique_word_count(message: str) -> int:
        message = ''.join(
            [char for char in message if char not in TEXTNOTCOUNTSYMBOLS])
